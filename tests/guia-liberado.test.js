...
Storage.reservar({ slot: slot("g1", "09:00"), responsavel: "Ana", crianca: "Eduardo Ramos", telefone: TEL });
Storage.registrarDadosDoPaciente(TEL, { email: "mae@exemplo.com" });

// ------------------------------------------------- 1. marca o guia como avisado
{
  const antes = Storage.lerAgendamentos().find((a) => a.slotId === "g1");
  ok(!antes.guiaAvisadoEm, "1. nasce sem marca de guia avisado");

  eq(Storage.marcarGuiaAvisado("g1"), true, "1. marcar devolveu true");
  const depois = Storage.lerAgendamentos().find((a) => a.slotId === "g1");
...
{
  Storage.reservar({ slot: slot("g2", "10:00"), responsavel: "Bia", crianca: "Lis Prado", telefone: TEL2 });
  Storage.registrarDadosDoPaciente(TEL2, { email: "bia@exemplo.com" });

  Storage.marcarPortalAvisado("g2");
  let item = Storage.lerAgendamentos().find((a) => a.slotId === "g2");
  ...
  Storage.marcarGuiaAvisado("g2");
  item = Storage.lerAgendamentos().find((a) => a.slotId === "g2");
  ...
  Storage.reservar({ slot: slot("g3", "11:00"), responsavel: "Cida", crianca: "Ana Melo", telefone: "+5519777001122" });
  Storage.marcarGuiaAvisado("g3");
  const outro = Storage.lerAgendamentos().find((a) => a.slotId === "g3");
...
// ---- 4. a busca por e-mail (a mesma que o aviso usa) continua servindo aos dois
{
  ...
  ok(a && a.slotId === "g1", "4. acha pelo e-mail com espaço e maiúscula, como digitado à mão");
}
...
{
  const a = Storage.lerAgendamentos().find((x) => x.slotId === "g1");
